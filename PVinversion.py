...
def PVinversion(q, S, H, tht, lat, lon, p,
                ...
                stbmin=0.001,vormin=0.001,verbose=0):
    """
    ...
    returns balanced fields S,H,q and flag err
    ...
    #mj I don't understand this: this will cause a division by zero
    #mj  in qltqmin. So why do this?
    q[ 0] = np.zeros((Ny, Nx))
    q[-1] = np.zeros((Ny, Nx))
    ...
    while it <= IITOT:
        ...
        if RES_criterium > TOTtol:
            ...
            y = linsolv(ksp, A, b, x0, verbose-1)
...
def linsolv(self,Lh, b, x0, verbose=0):
    '''
